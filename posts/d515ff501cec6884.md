...
> In this video, AWS CEO Matt Garman announces AWS has grown to a $132 billion business with 20% year-over-year growth. He unveils major AI infrastructure advances including P6e-GB300 instances with NVIDIA GB300, AWS AI Factories for dedicated on-premises AI infrastructure, and Trainium3 UltraServers delivering 4.4x more compute. Key launches include Amazon Nova 2 model family (Lite, Pro, Sonic, Omni), Nova Forge for open training models creating custom "Novellas," and Amazon Bedrock AgentCore enhancements with Policy and Evaluations. Three frontier agents debut: Kiro autonomous agent for software development, AWS Security Agent for proactive security testing, and AWS DevOps Agent for incident resolution. Additional announcements include AWS Transform Custom for code modernization, Amazon Quick enterprise AI assistant, and 25 core service updates spanning compute (new X, C8a, M8azn instances), storage (S3 50TB object limit, S3 Vectors GA, S3 Tables Intelligent-Tiering), databases (RDS 256TB storage, Database Savings Plans), and security (enhanced Security Hub, unified CloudWatch data store). Sony and Adobe executives share their AWS transformation stories, demonstrating real-world AI agent implementations.
...
### Welcome to re:Invent 2025: AWS Reaches $132 Billion with Accelerating Growth
...
### Amazon Quick: Empowering Every Employee with AI-Powered Insights
...
Take, for one example, our internal Amazon tax team, where they built a Quick agent that helps them consolidate all of their sources of tax data, whether they're projects from audits or details from the internet. It performs deep research into any tax code changes or policy changes that might be made, and it presents all of this tax information from all those sources of data in a single view for them. They then use Quick to visualize that information, and it allows them to track regulatory changes in real time.
