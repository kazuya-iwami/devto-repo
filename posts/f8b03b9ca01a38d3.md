...
So, in Amazon SageMaker Unified Studio, when we find these data products or data assets,  there's a very quick way that we can promote them into Amazon QuickSight, very quick, it's just one click. Yeah, and Amazon QuickSight is actually part of the resource provisioning stage that we talked about earlier. This is all blueprinted and it comes with the product. So it's one click onboarding.
