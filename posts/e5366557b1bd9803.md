...
Really quickly before we go to questions, one of the things I want to point out is that when we went through the solution for the database migration, there was a step that we left out due to time—the how do we get from here to there.  One of the things that we left out, and this is where a partnership comes in, is that Datastore is not an exact match to DynamoDB. Harleen, what are some of the requirements that The New York Times gave us, like transaction rates?
