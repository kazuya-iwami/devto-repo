...
So the most ubiquitous pattern right now in agents is the manager agent and the sub agents. The manager agent dictates when and who it calls to get work done and it decides when the work is done. There's also graph agents. You can think of it as a left to right graph. In our case for coding, every agent is dependent on the previous node—they call them nodes.
